package gov.va.vinci.menopause.listeners;
...
public class ListenerLogic {
    ...
    public static HashMap<String, String> getInstanceData(JCas aJCas, Annotation currAnnotation) throws CASException {
        ...
        currentDoc.put("Snippets", shortenText(currAnnotation.getCoveredText(), 400, true));
        ...
        for (int i = 0; i < featuresList.size(); ++i) {
            ...
            if (typeRange.isPrimitive()) {
                ...
                if (fs instanceof Annotation) {
                    try{
                        if( fs instanceof AnchoredSentence) {
                            AnchoredSentence anchoredSentence = (AnchoredSentence) fs;
                            Annotation anchor = anchoredSentence.getAnchor();
                            String concept = anchor.getFeatureValueAsString(anchor.getType().getFeatureByBaseName("concept"));
                            currentDoc.put("concept", concept);
                        }
                    } catch(Exception e){
                        // do nothing. Just swallow.
                        // TODO: do something.
                    }
                    currentDoc.put(feature.getShortName(), shortenText(((Annotation)annotation.getFeatureValue(feature)).getCoveredText(),400 , true ));
